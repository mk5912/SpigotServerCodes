...
if %skip%==false (
	...
	if not exist portmap.jar (
		...
		if !errorlevel! equ 1 (start !link!) else (
			...
			echo.This Can Be Changed In The batch.settings File To Re-enable This Check.&&timeout 3 /nobreak>nul
		)
	)
)
cls&exit /b 0
...
set /a sE=%sE%+1
echo.Error: %~1&&echo.[%time:~0,-3%] Error.Name: %~1>>logs\latest.log
echo.Tag: %~2&&echo.[%time:~0,-3%] Error.Tag: %~2>>logs\latest.log
echo.Value: %~3&&echo.[%time:~0,-3%] Error.Value: %~3>>logs\latest.log
if exist error-Codes.txt (
	for /f "tokens=2 delims=-" %%e in ('find /i "%~2" error-Codes.txt') do set des=%%e
	if "!des!" neq "" (echo.Description:!des!&&echo.[%time:~0,-3%] Error.Description:!des!>>logs\latest.log)
)
pause
exit