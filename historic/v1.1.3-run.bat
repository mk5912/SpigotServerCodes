...
rem call :batchRefresh defBack %backup%
call :error ending
...
if "%end%"=="" set /p ver=No Version Found, Please Input A Valid Version: 
...
if %port% lss 10000 (call :error invalid_Port , 0xF3 , %pt%)
for /l %%i in (0,1,4) do (
...
for /f "tokens=*" %%b in ('find /i "backup-Location" batch.settings') do set locBack=%%b
set ver=%ver:~8%
set skip=%skip:~10%
...
:batchRefresh
if %~1==ver (echo.version=%~2>batch.settings&&echo.skip-UPnP=!skip!>>batch.settings& echo.backup-Default=!defBack!>>batch.settings&&echo.backup-Location=!locBack!>>batch.settings) || call :error bat_Settings , 0xA8 , update_FAIL
if %~1==skip (set skip=%~2&& echo.version=!ver!>batch.settings&& echo.skip-UPnP=%~2>>batch.settings&& echo.backup-Default=!defBack!>>batch.settings&&echo.backup-Location=!locBack!>>batch.settings) || call :error bat_Settings , 0xA8 , update_FAIL
if %~1==defBack (set defBack=%~2&& echo.version=!ver!>batch.settings&& echo.skip-UPnP=!skip!>>batch.settings&& echo.backup-Default=%~2>>batch.settings&&echo.backup-Location=!locBack!>>batch.settings) || call :error bat_Settings , 0xA8 , update_FAIL
if %~1==locBack (set locBack=%~2&& echo.version=!ver!>batch.settings&& echo.skip-UPnP=!skip!>>batch.settings&& echo.backup-Default=!defBack!>>batch.settings&&echo.backup-Location=%~2>>batch.settings) || call :error bat_Settings , 0xA8 , update_FAIL
exit /b 0
...
if exist error-Codes.txt (
	...
	set des=!des:~7!
	echo.Description: !des! &&echo.[%time:~0,-3%] System.Description: !des! >>logs\latest.log
)
pause
if %~3==rename_FAIL goto :end
call :log
